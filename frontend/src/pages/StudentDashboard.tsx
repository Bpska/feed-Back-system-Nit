...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    loadData();
    // Real-time subscription removed for local API version
  }, []);
  ...
  const loadData = async () => {
    try {
      ...
      const assignmentsData = await api.get(
        `/ratings/assignments?year=${profileData.year}&semester=${profileData.semester}&branch=${profileData.branch}&section=${profileData.section}`
      );
      setFacultyAssignments(assignmentsData);

      // Load messages
      await loadMessages(profileData.id);
  ...
  const loadMessages = async (userId: string) => {
    try {
      ...
      // Mark unread messages as read - implemented server-side if needed, or by client calls
      // For now, assuming server handles it or we call specific endpoint if defined
      // The backend has POST /messages/:messageId/read
    } catch (error: any) {
                      ...
                      <div key={rating.id} className="p-4 bg-muted/50 rounded-lg border">
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">{rating.assignment?.subject}</p>
                          </div>
